//+------------------------------------------------------------------+
//|                                      Python_Bridge_EA_Multi.mq5   |
//|                  Python ML Bridge - Multi-Symbol Signal Executor    |
//|                                                                    |
//|  Multi-symbol version of the Python ML Bridge EA. Works on ANY     |
//|  symbol (EURUSD, GBPUSD, USDJPY, AUDUSD, etc.) - not limited to   |
//|  XAUUSD/Gold. Uses separate signal/status/heartbeat files so it    |
//|  can run alongside the original Gold-only Python_Bridge_EA.        |
//|                                                                    |
//|  Communication Protocol:                                           |
//|    Python -> MT5: python_bridge_signal_multi.csv (signals)          |
//|    MT5 -> Python: python_bridge_confirm_multi.csv (confirmations)   |
//+------------------------------------------------------------------+
#property copyright "Python ML Bridge - Multi Symbol"
...
//| Input Parameters                                                   |
//+------------------------------------------------------------------+
input string   InpSignalFile       = "python_bridge_signal_multi.csv";   // Signal file name
input string   InpConfirmFile      = "python_bridge_confirm_multi.csv";  // Confirmation file name
input string   InpExitFile         = "python_bridge_exit_multi.csv";     // Exit signal file name
input string   InpHeartbeatFile    = "python_bridge_heartbeat_multi.txt"; // Heartbeat file name
...
input int      InpMagicNumber      = 20240116;  // Magic number for orders
...
input string   InpStatusFile       = "python_bridge_status_multi.txt"; // Status file name
...
bool ValidateSignal()
{
    ...
    // Symbol check removed - this multi-symbol EA works on ANY symbol
...
void ExecuteSignal()
{
    ...
    // For 5-digit brokers: 1 pip = 10 points (works for gold and forex)
...
void ManageOpenPositions()
{
    int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
    double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
    double pipValue = point * 10;  // 1 pip = 10 points for 5-digit brokers
    datetime currentTime = TimeCurrent();
...
void UpdateDashboard()
{
    ...
    // --- Title ---
    DashboardLabel("title", panelX + leftMargin, y, "PYTHON ML BRIDGE - MULTI SYMBOL", clrTitle, 10, "Consolas Bold");
    y += 28;
