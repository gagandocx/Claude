    ...
    def run_retrain(self, train_fn=None, evaluate_fn=None) -> Dict:
        ...
        try:
            ...
            if train_fn:
                ...
                # Import train_all from the neurox package.
                # train.py lives at the neurox package root, which is
                # one directory up from this file (training/auto_retrain.py).
                train_all = None

                # Try 1: Absolute package import (works if neurox is on sys.path)
                try:
                    from neurox.train import train_all as _ta
                    train_all = _ta
                except ImportError:
                    pass

                # Try 2: Direct relative import (works when running from neurox/)
                ...
                if train_all is None:
                    ...
                    # From training/auto_retrain.py, go up one level to neurox/
