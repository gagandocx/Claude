...
from strategies.circuit_breaker import GroupthinkCircuitBreaker, CircuitBreakerConfig
...
# ─────────────────────────────────────────────
#  LOGGING SETUP (with rotation: 10MB / 5 backups)
# ─────────────────────────────────────────────
def setup_logging(config: MainConfig) -> logging.Logger:
    """Configure logging with rotation (10MB max, 5 backup files)."""
...
class PythonMLBridge:
    ...
    def __init__(self, config: Optional[MainConfig] = None):
        ...
        # Circuit breaker for groupthink detection (correlated model failures)
        self.circuit_breaker = GroupthinkCircuitBreaker(CircuitBreakerConfig())

        # Notification system (opt-in, disabled by default)
        self.notifier = NotificationManager(NotificationConfig())

        # Position reconciliation on startup
        from config.settings import MT5_COMMON_PATH
        self.reconciler = PositionReconciler(MT5_COMMON_PATH)
        ...
        # v2: Position reconciliation on startup (solves "ghost HOLD" problem)
        reconciled = self.reconciler.reconcile()
        if reconciled:
            self.signal_generator._active_position = reconciled
            self.logger.info(
                f"  Position reconciled: {reconciled['direction']} "
        ...
        self.logger.info(f"  Circuit Breaker: ON (groupthink detection)")
        self.logger.info(f"  Notifications:   {self.notifier.config.enabled}")
        self.logger.info(f"  Position Reconciliation: ON")
